...
namespace EldritchArcana
{
    internal class FaithTraits
    {
        public static BlueprintFeatureSelection CreateFaithTraits()
        {
            ...
            var IomedaeFeature = Traits.library.Get<BlueprintFeature>("88d5da04361b16746bf5b65795e0c38c");
            var requiredgod = Helpers.PrerequisiteFeature(IomedaeFeature);
            var components = new List<BlueprintComponent> { requiredgod };
            components.Add(Helpers.CreateAddStatBonus(StatType.SaveWill, 1, ModifierDescriptor.Trait));
            components.Add(Helpers.Create<SavingThrowBonusAgainstDescriptor>(a =>
            {
                a.SpellDescriptor = SpellDescriptor.Evil;
                a.Value = 1;
                a.ModifierDescriptor = ModifierDescriptor.Trait;
            }));
            choices.Add(Helpers.CreateFeature("PurityOfFathTrait", "Purity of Faith(Iomedae)",
                "Your soul is free from impurity, and you are deeply committed to fulfilling your duties to the church.\nBenefit: You gain a +1 trait bonus to Will saves and a +1 trait bonus to saving throws versus saves vs spells cast with the evil descriptor.",
                "72b92d2ebb4c4cdb8e946e29f5b2f162",
